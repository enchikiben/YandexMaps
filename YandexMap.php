...
EQF;
    ...
    protected function polyline($name, &$value)
    {
        ...
        $polyline .= "var $name = new ymaps.Polyline([" . implode(
                ',',
                $coordinates
            ) . "], {" . $properties . "}, {" . $options . "});";
    ...
    protected function balloons()
    {

        $balloon = '';

        $balloon .= $this->balloon('balloon', $this->balloon);

        return $balloon;
    }

    protected function balloon($name, &$value)
    {
        if (!isset($value['lat'], $value['lon'])) {
            return;
        }
        ...
        $balloon .= "map.balloon.open([{$value['lat']},{$value['lon']}], '{$value['content']}',{" . $options . "});";
